...
type: spec
status: active
tags: [embassies, ambassadors, diplomacy, cross-simulation]
---
...
Embassies are a cross-simulation feature that establishes physical diplomatic presences between connected simulations. Each embassy is a special building in one simulation that represents another simulation, staffed by an ambassador agent. Embassies create tangible, in-world anchors for the Bleed mechanic described in `../explanations/concept-lore.md` (Chapter VII: The Vectors — Architecture).
...
|---------|-------------|
| **Simulation Connections** (`relationships-echoes-map.md`) | Embassies are the in-world manifestation of simulation connections |
...
| **Per-Simulation Theming** (`theming-system.md`) | Embassy visual effects use theme colors for per-world identity |
