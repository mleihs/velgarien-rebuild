-- Migration 069: Event Lifecycle — Status, Escalation, and Decay
--
-- Adds:
--   1. event_status column on events table (active, escalating, resolving, resolved, archived)
--   2. event_chains table for linking related events (escalation, follow_up, resolution)
--   3. Updated zone_event_pressure CTE with status multiplier
--
-- Status pressure multipliers:
--   active     = 1.0x (default, currently affecting stability)
--   escalating = 1.3x (worsening — impact rising)
--   resolving  = 0.5x (being addressed — impact decreasing)
--   resolved   = 0.0x (no longer affects stability)
--   archived   = 0.0x (historical record)

-- ============================================================================
-- 1. ADD event_status COLUMN TO events
-- ============================================================================

ALTER TABLE events
  ADD COLUMN IF NOT EXISTS event_status text NOT NULL DEFAULT 'active';

-- Validate values via CHECK constraint
ALTER TABLE events
  ADD CONSTRAINT chk_event_status
  CHECK (event_status IN ('active', 'escalating', 'resolving', 'resolved', 'archived'));

CREATE INDEX IF NOT EXISTS idx_events_status ON events (event_status);

COMMENT ON COLUMN events.event_status IS
  'Lifecycle state: active (default), escalating (worsening), resolving (being addressed), '
  'resolved (no longer affects stability), archived (historical record).';


-- ============================================================================
-- 2. CREATE event_chains TABLE
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS event_chains (
  ...
  CONSTRAINT chk_chain_type CHECK (chain_type IN ('escalation', 'follow_up', 'resolution')),
  CONSTRAINT chk_no_self_chain CHECK (parent_event_id != child_event_id),
  CONSTRAINT uq_event_chain UNIQUE (parent_event_id, child_event_id, chain_type)
);

CREATE INDEX idx_event_chains_parent ON event_chains (parent_event_id);
CREATE INDEX idx_event_chains_child ON event_chains (child_event_id);
CREATE INDEX idx_event_chains_sim ON event_chains (simulation_id);

COMMENT ON TABLE event_chains IS
  'Links related events into narrative arcs. chain_type determines the relationship: '
  'escalation (crisis deepening), follow_up (related development), resolution (event addressed).';

-- RLS for event_chains
ALTER TABLE event_chains ENABLE ROW LEVEL SECURITY;

CREATE POLICY "event_chains_read_all" ON event_chains
  FOR SELECT USING (true);

CREATE POLICY "event_chains_insert_authenticated" ON event_chains
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "event_chains_delete_authenticated" ON event_chains
  FOR DELETE USING (auth.role() = 'authenticated');


-- ============================================================================
-- 3. REBUILD mv_zone_stability WITH STATUS MULTIPLIER
-- ============================================================================
-- Must drop and recreate due to the new CTE logic incorporating event_status.
...
-- ============================================================================
-- 4. RECREATE mv_simulation_health (unchanged, dependency rebuild)
-- ============================================================================
