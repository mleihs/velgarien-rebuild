...
backend/              FastAPI application
  ...
  routers/            API endpoints — 32 routers, 246 endpoints (/api/v1/... + /api/v1/public/... + /api/v1/admin/...)
  models/             Pydantic request/response models (21 files)
  services/           Business logic (BaseService + 28 entity + audit + simulation + external + email + admin + bot + notification)
  ...
  tests/              pytest tests (716 tests: unit + integration + security + performance)
...
supabase/
  migrations/         44 SQL migration files (001-037 + ensure_dev_user + 040 platform_settings + 041 bot_players + 042 rls_hardening + 043 cite_des_dames + 044 notification_preferences)
...
- **20 functions** — RLS helpers, utility (generate_slug, validate_taxonomy_value), trigger functions, admin RPC (admin_list_users, admin_get_user, admin_delete_user via SECURITY DEFINER), notification RPC (get_user_emails_batch via SECURITY DEFINER)
...
| Doc | Content | Version |
|-----|---------|---------|
| `docs/03_DATABASE_SCHEMA_NEW.md` | Complete schema (45 tables, triggers, views, RLS) | v2.8 |
| `docs/04_DOMAIN_MODELS.md` | TypeScript interfaces (aligned with schema v2.8) | v2.8 |
| `docs/05_API_SPECIFICATION.md` | 246 endpoints with request/response formats | v1.8 |
| `docs/07_FRONTEND_COMPONENTS.md` | Component hierarchy, routing, state management | v2.0 |
